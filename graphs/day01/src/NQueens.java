...
public class NQueens {
    ...
    public static List<char[][]> nQueensSolutions(int n) {
        ...
        helper(board, 0, checkcol, n,answers);
        return answers;
    }

    //
    public static void helper(char[][] board, int col, boolean[] chekcol, int n, List<char[][]> answers){
        if (col >= n){
            answers.add(copyOf(board));
            return;}

        for(int i = 0; i < n; i++){
            if (!checkDiagonal(board, col, i) && !chekcol[i] ){
                board[col][i] = 'Q';
                chekcol[i] = true;
//                if (helper(board,col +1,n,answers) == true){
//                    return;
//                }
                helper(board,col +1, chekcol, n, answers);
